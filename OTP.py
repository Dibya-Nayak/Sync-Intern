...
password = "aezjmddcdaduvmjt"  # password to get into the sender's gmail account
server.login("djnayak001@gmail.com", password)  # logging into the sender's gmail account

body = "Dear " + name + "," + "\n" + "\n" + "Your One Time Password (OTP) is " + str(OTP) + "."  # generating a message
subject = "One Time Password (OTP) for verification"
message = f'Subject:{subject}\n\n{body}'

server.sendmail("djnayak001@gmail.com", valid_receiver_email, message)


def sending_otp(receiver_email):
    New_OTP = random.randint(100000, 999999)

    body = "Dear " + name + "," + "\n" + "\n" + "Your One Time Password (OTP) is " + str(
        New_OTP) + "."  # generating a message with new OTP
    subject = "One Time Password (OTP) for verification"
    message = f'Subject:{subject}\n\n{body}'

    server.sendmail("djnayak001@gmail.com", receiver_email, message)
